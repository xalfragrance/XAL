...
local XALHubConfig = Window.ConfigManager:CreateConfig("xalhub")
...
local function Reg(id, element)
    XALHubConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "XAL Hub") .. "/config/"
...
do
    ...
    local function CreateKaitunUI()
        ...
        txt("KAITUN XAL HUB (PREMIUM)", 0.2, Color3.fromRGB(255,0,255), 35)
        ...
        return {Gui=sg, Labels={Coins=lCoins, LastCatch=lLC, Gear=lGear, Status=lStat}}
    end
    ...
    local function RunFarmingFish(dynamicDelay)
            ...
            while KAITUN_ACTIVE do
                ...
                if step <= 4 then
                    ...
                    if coins >= targetPrice then
                        ...
                        RunFarmingFish(CURRENT_KAITUN_DELAY)
                    end

                elseif false then -- step 5 disabled by request
                ...
                elseif false then -- step 6 disabled by request
                    ...
                    uiData.Labels.Status.Text = "Farming (Max Gear)"
                    local hrp = game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if hrp and (hrp.Position - ENCHANT_ROOM_POS).Magnitude > 10 then
                            TeleportToLookAt(ENCHANT_ROOM_POS, ENCHANT_ROOM_LOOK)
                            task.wait(0.5)
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "AL Notify" 
    ...
    local GLOBAL_WEBHOOK_URL = "https://discord.com/api/webhooks/1449824296334786790/meqYs1vevNsWxGLpP56UdiX3shnEV672i5yfScFQdCRaWxl2EZuu1q-IxQl74OQg25DN"
    local GLOBAL_WEBHOOK_USERNAME = "XAL FRAGRANCE"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("XAL | Fish it\nOlih iwak lur! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("XAL Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> XAL Hub | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("XAL Hub Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[XAL Hub Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "xalhub" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "XAL Hub") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            -- 1. Save ke config utama dulu ("xalhub.json")
            XALHubConfig:Save()
            task.wait(0.1)

            -- 2. Jika nama beda, salin isi "xalhub.json" ke "NamaBaru.json"
            if SelectedConfigName ~= "xalhub" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "xalhub.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "xalhub" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("xalhub")
                SelectedConfigName = "xalhub"
            else
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_XALHub")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_XALHub"
